...
const AppointmentManager = () => {
  ...
  const appointmentsForSelectedDate = selectedDate ? appointments.filter(app => app.date === format(selectedDate, "yyyy-MM-dd")) : [];

  // Filtrer les rendez-vous à venir et passés pour les onglets
  const upcomingAppointments = appointments.filter(app => app.date >= todayDateString);
  const pastAppointments = appointments.filter(app => app.date < todayDateString);
  ...
  const generateGoogleCalendarLink = (appointment: Appointment) => {
    ...
    const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(text)}&dates=${startFormatted}/${endFormatted}&details=${encodeURIComponent(details)}`;
  ...
  const generateOutlookCalendarLink = (appointment: Appointment) => {
    ...
    const outlookCalendarUrl = `https://outlook.office.com/calendar/0/deeplink/compose?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}&startdt=${startFormatted}&enddt=${endFormatted}`;
    return outlookCalendarUrl;
  };
  return <div className="space-y-6 animate-in fade-in-0">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">Gestion des rendez-vous</h1>
        <Button onClick={() => setExportModalOpen(true)} className="flex items-center gap-2 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <TabsContent value="upcoming" className="mt-0">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {upcomingAppointments.length === 0 && <TableRow>
              ...
              <TabsContent value="past" className="mt-0">
                <Table>
                  ...
                  <TableBody>
                    ...
                    {pastAppointments.length === 0 && <TableRow>
                        <TableCell colSpan={4} className="text-center py-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Calendar mode="single" selected={selectedDate} onSelect={setSelectedDate} locale={fr} className="pointer-events-auto py-0 px-[2px]" />
              
              {selectedDate && <div className="mt-4">
                  <h3 className="text-sm font-medium mb-2">
                    Rendez-vous du {format(selectedDate, "d MMMM yyyy", {
                  locale: fr
                })}
                  </h3>
                  
                  {appointmentsForSelectedDate.length > 0 ? <div className="space-y-2">
                      {appointmentsForSelectedDate.map(appointment => <div key={appointment.id} className="p-2 bg-secondary/10 rounded-md text-sm">
                          <div className="flex justify-between items-center">
                            <p className="font-medium">{appointment.heure}</p>
                            <p className={`px-2 py-0.5 rounded-full text-xs ${appointment.statut === "à venir" ? "bg-primary/10 text-primary" : "bg-muted text-muted-foreground"}`}>
                              {appointment.statut}
                            </p>
                          </div>
                          <p>{appointment.nom}</p>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              ...
              const csvData = appointments.map(appointment => [appointment.nom, appointment.email, appointment.date, appointment.heure, appointment.motif, appointment.statut]);
